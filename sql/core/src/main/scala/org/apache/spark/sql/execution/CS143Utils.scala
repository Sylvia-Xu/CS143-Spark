  ...
  def apply(
    ...
    { input =>
      new Iterator[Row] {
        ...
        // This is a special iterator, which points to the same list as the regular list iterator. The difference is that
        // we will use the regular iterator to fetch data, while using this iterator to do caching for all the rows
        // in this list

        // the function of hasNext() is: is there any more rows that need to be cached?
        def hasNext() = {
          // IMPLEMENT ME
        ...
        // the function of next() is: do caching for the next row in the array
        def next() = {
          // IMPLEMENT ME
