...
protected [sql] final class GeneralDiskHashedRelation(partitions: Array[DiskPartition])
  ...
  override def closeAllPartitions() = {
    // IMPLEMENT ME
    partitions.map(x=>x.closePartition())
  }
}
...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    ...
    data.add(row)
    val newlyAdded = data.size()-1
    val newSize = measurePartitionSize()
    if(newSize > blockSize){
      data.remove(newlyAdded)
  ...
  def getData(): Iterator[Row] = {
    ...
    new Iterator[Row] {
      ...
      override def next() = {
        // IMPLEMENT ME
//        val hasMoreRow: Boolean = hasNext()
        val nextRow: Row = if(hasNext()) {
      ...
      override def hasNext() = {
        // IMPLEMENT ME
//        val hasMoreRow: Boolean = currentIterator.hasNext()
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    var partitionNum: Array[Int] = hashValue.map(_ % size)

    var partitions: Array[DiskPartition] = new Array[DiskPartition](size)

    var x: Int = 0
    for (x<- 0 to size-1){
      var filename: String = x.toString()
      var newPartition: DiskPartition = new DiskPartition(filename, blockSize)
      partitions(x) = newPartition
    }

    var a: Int = 0
    for (a<- 0 to data.size-1){
      var index: Int = partitionNum(a)
      partitions(index).insert(data(a))
    }
